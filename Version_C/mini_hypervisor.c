			...
			else if (v.run->io.port == 0x0278)
			{
				...
				if (v.run->io.direction == KVM_EXIT_IO_OUT)
				{
					...
					case STATE_OPEN_WANT_PATH_BYTE:
						...
						break;
					case STATE_WRITE_WANT_VFD:
						hc_write_vfd = (int)out_val;
						fcall_state = STATE_WRITE_WANT_COUNT;
						break;

					case STATE_WRITE_WANT_COUNT:
					{
						int vfd = hc_write_vfd;
						hc_write_len = (int)out_val; // How many bytes to expect
						...
						if (hc_write_len == 0 ||
							...
							!my_file_table[vfd].in_use)
						{
							hc_ret_val = (hc_write_len == 0) ? 0 : -1; // Error or 0 bytes
							fcall_state = STATE_IDLE;				   // Abort
						}
						else
						{
							// Valid, move to next state to receive bytes
							fcall_state = STATE_WRITE_WANT_BYTE;
						}
						break;
					}

					case STATE_WRITE_WANT_BYTE:
					{
						int vfd = hc_write_vfd;
						FILE *f = my_file_table[vfd].host_fd;

						// Write the byte directly to the file
						size_t bytes_written = fwrite(&out_val, 1, 1, f);
						hc_write_idx++;
						...
						else if (hc_write_idx == hc_write_len)
						{
							// This was the last byte, we are done
							hc_ret_val = hc_write_len; // Success, return bytes written
							fcall_state = STATE_IDLE;
						}
						// else: loop in this state until all bytes are received
						break;
					}
